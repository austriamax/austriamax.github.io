...
\[\begin{align*}
a^tMa &= \mathbb{E}[a^t(X-\mathbb{E}X)(X-\mathbb{E}X)^ta]\\\\
&= \mathbb{E}[\{a^t(X-\mathbb{E}X)\}\{a^t(X-\mathbb{E}X)^t\}]\\\\
\end{align*}\]

Como \[\{a^t(X-\mathbb{E}X)\}\{a^t(X-\mathbb{E}X)^t\}\] es una v.a. no negativa, entonces por la monótonia de la esperanza se sique que 
\[a^tMa \geq 0\]

\[\therefore M \mbox{ es semidefinida positiva.}\]
